...
const cris1 = require("discord.js");
const cris2 = require("discord.js");
const cris3 = require("discord.js");

const client1 = new cris1.Client();
const client2 = new cris2.Client();
const client3 = new cris3.Client();

const ayarlar = require('./ayarlar.json');
const config = require('./cris.json');
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: " cris Efsanesi System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
  ...
  channel.send(`<@619126609756160000>`, {embed: fayik}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: " cris Efsanesi System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: " cris Efsanesi System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: " cris Efsanesi System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: ` cris Efsanesi System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: ` cris Efsanesi System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: ` cris Efsanesi System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: " cris Efsanesi System | Rol Koruma Sistemi" }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `cris System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "cris System | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: ` cris Efsanesi System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: " cris Efsanesi System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(member.id, { reason: ` cris Efsanesi System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: " cris Efsanesi System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: " cris Efsanesi System | İzinsiz Webhook Açmak!"}).catch(err => { });
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`<@619126609756160000>`, {embed: fayik}).catch(err => { })
return;
});
...
/// discord.gg/codeart/// discord.gg/codeart/// discord.gg/codeart/// discord.gg/codeart/// discord.gg/codeart/// discord.gg/codeart/// discord.gg/codeart////////////
